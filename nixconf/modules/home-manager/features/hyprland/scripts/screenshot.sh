...
Modes:
  ...
  screen        take screenshot of the current monitor
  full          take screenshot of all monitors combined
...
function send_notification() {
    ...
    local recentFile=$(find "$outputDir" -name 'screenshot_*.png' -printf '%T+ %p\n' | sort -r | head -n 1 | cut -d' ' -f2-)
...
function begin_grab() {
    ...
    case $option in
	    ...
	    geometry=`trim "${geometry}"`
            ;;
        screen)
           local geometry=`grab_output_current_monitor`
            ;;
        full)
            local geometry=$(grab_full)
            ;;
    esac
...
function grab_output_current_monitor() {
    mouse_pos=$(hyprctl cursorpos)
    mouse_x=$(echo "$mouse_pos" | awk -F'[, ]+' '{print $1}')
    mouse_y=$(echo "$mouse_pos" | awk -F'[, ]+' '{print $2}')

    local monitor=$(hyprctl monitors -j | jq -r \
        --arg mx "$mouse_x" \
        --arg my "$mouse_y" \
        '.[] | select(
            (.x <= ($mx | tonumber)) and 
            (.x + (.width / .scale | round) >= ($mx | tonumber)) and 
            (.y <= ($my | tonumber)) and 
            (.y + (.height / .scale | round) >= ($my | tonumber))
...
function grab_full() {
    local geometry=$(hyprctl monitors -j | jq -r '
        # Calcular min_x, min_y, max_x, max_y manualmente
        reduce .[] as $mon (
            { min_x: infinite, min_y: infinite, max_x: -infinite, max_y: -infinite };
            .min_x = ([.min_x, $mon.x | tonumber] | min) |
            .min_y = ([.min_y, $mon.y | tonumber] | min) |
            .max_x = ([.max_x, ($mon.x + $mon.width) | tonumber] | max) |
            .max_y = ([.max_y, ($mon.y + $mon.height) | tonumber] | max)
        ) |
        "\(.min_x),\(.min_y) \(.max_x - .min_x)x\(.max_y - .min_y)"
    ')
    echo "$geometry"
}
...
function parse_mode() {
    ...
    case $mode in
        window | region | output | screen | full)
            OPTION=$mode
            ;;
        active)
            CURRENT=1
...
[ -z "$outputDir" ] && SAVEDIR=${XDG_PICTURES_DIR:=~} || SAVEDIR=${outputDir}
